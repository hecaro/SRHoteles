...
$consulta=mysql_query("SELECT * FROM  `REGISTRO_HUESPED` INNER JOIN  `USUARIO` ON  `hotel` =  `hotelid` INNER JOIN `HABITACIONES` ON `idhotel`=`hotel` WHERE  `ident` =  '$busqid' AND `rsocial`='$usuario' LIMIT 0 , 30");
...
if ($fila){
	...
	echo "<h3>TIPO DE HABITACION / CANTIDAD: SIMPLE	
		...
		echo "<h3>ESTATUS HABITACION:
			...
			<input type=radio name=status id=stat value=libre> LIBERADO
			</h3>";
