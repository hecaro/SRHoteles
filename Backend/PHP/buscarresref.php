...
//cuando de: PHP Notice:  Undefined variable: pasifrprint in... utilizar este codigo para verificar la variable
 $busqid=isset($_POST['referenciahuesped']) ? $_POST['referenciahuesped'] : "";
...
$consulta=mysql_query("SELECT * FROM  `REGISTRO_HUESPED` INNER JOIN  `USUARIO` ON  `hotel` =  `hotelid` INNER JOIN `HABITACIONES` ON `idhotel`=`hotel` WHERE  `referencia` =  '$busqid' AND `rsocial`='$usuario' LIMIT 0 , 30");
